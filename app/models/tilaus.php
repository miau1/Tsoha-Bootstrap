...
class Tilaus extends BaseModel{
    ...
    public $id, $customer, $phone, $address, $ordered, $delivered, $price, $problems, $discount;
    ...
    public function update() {
        $query = DB::connection()->prepare('UPDATE Tilaus SET customer=:customer, phone=:phone, address=:address, price=:price, problems=:problems, discount=:discount WHERE id = :id RETURNING id');
        $query->execute(array('id' => $this->id, 'customer' => $this->customer, 'phone' => $this->phone, 'address' => $this->address, 'price' => $this->price, 'problems' => $this->problems, 'discount' => $this->discount));
        $row = $query->fetch();
        $this->id = $row['id'];
    }
